...
我在做周期性回顾的时候发现了一个 bug：写了上千行 System Prompt 规则，很多该触发的原则根本没有触发。不是规则写得不对，是 agent 的指令遵从能力在 System Prompt 膨胀的过程中持续下降——积累了很多东西，但没有真正执行。

这个 bug 让我意识到问题不在规则本身，是整个架构不对。

后来反复折腾了三周，砍掉 90% 的设计，最终剩下 5 个 bash 函数、140 行代码：**[agent-assembly](https://github.com/LPASK/agent-assembly)**。它解决的是使用 Claude Code 跨项目进行[复利工程](https://github.com/EveryInc/compound-engineering-plugin)的痛点——你的技术背景、认知模式、决策偏好、当前目标，这些属于"你"而不是某个项目的信息，换个项目就全丢了。
...
最初所有东西都在一个项目里——skills、个人画像、认知模式、记忆系统、feed 处理流程，全堆进配置。System Prompt 越积越长，agent 基于残缺信息做出错误判断。这就是单一仓库的结构性问题：只会膨胀，不会收缩。

拆出来是显然的方向，但怎么拆？这里有一个两难：

- **塞进 System Prompt**：不会被压缩遗忘，但越长指令遵从越差，而且只会越积越多。
- **不塞 System Prompt**：保持精简，但内容在长对话中被压缩丢失，模型随时遗忘。

全塞不行，全不塞也不行。需要一个机制在启动前就决定好**什么该进、什么不该进**。

## 踩坑

方向有了——拆分模块，按需注入。但实现的时候踩了一堆坑。

**Plugin**。最直觉的方案：做一个 Plugin 装到每个项目，自动注入 profile。试了之后发现 Plugin 机制注入不了 System Prompt。路不通。
...
**过度抽象**。system prompt 放什么、startup 读什么、hook 注入什么——一个列表就够了。但我设计了四层上下文模型和各种 Registry。在周期性回顾的时候发现，这些抽象对结果没有帮助。瓶颈在人——东西越复杂越难驾驭，越简单越好驾驭。每砍一个"精心设计"的模块，系统反而变好用了。
...
在现有框架内构建省掉了所有不必要的麻烦：不需要用户配 Provider，不需要维护基础设施，新功能搭框架的顺风车就行。
...
三周下来最大的发现：**agent 系统跟传统软件不一样——你只要把事情讲清楚，模型就能理解。** 上千行规则、四层上下文模型、独立 Manager Agent——这些都是在用系统复杂度去补人的判断力。但复杂的架构不会让模型理解得更好，只会让你自己更难维护。瓶颈不在系统，在人。
